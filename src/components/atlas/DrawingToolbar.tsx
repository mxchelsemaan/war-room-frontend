...
export function DrawingToolbar({
  ...
  // Saved-item selection color (pushed up from SavedItemsContent)
  const selSetColorRef = useRef<((c: string) => void) | null>(null);
  const [selColor, setSelColor] = useState<string | null>(null);

  const handleSelectionColor = useCallback((c: string | null, setter: ((c: string) => void) | null) => {
    selSetColorRef.current = setter;
    setSelColor(c);
  }, []);

  // Unified color: selected saved item overrides creation color
  const hasSavedSelection = selColor !== null;
  const activeColor = hasSavedSelection ? selColor : (placementMode ? pendingColor : color);
  const isPinContext = mode === "pin";

  function setActiveColor(c: string) {
    if (hasSavedSelection && selSetColorRef.current) {
      selSetColorRef.current(c);
    ...
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 w-96">
      <CollapsePanel open={open} direction="up">
        ...
        <div className="border-t border-border/30 pt-2">
          <SavedItemsContent onSelectionColor={handleSelectionColor} />
        </div>
        </div>
      </CollapsePanel>
    </div>
