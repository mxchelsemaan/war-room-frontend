-- Add free-text search (p_query) to get_feed_events.
-- Uses ILIKE for simplicity — works across JSONB fields without tsvector indexes.

-- Drop the old 11-param overload (without p_query) to avoid PostgREST 300 ambiguity
DROP FUNCTION IF EXISTS public.get_feed_events(text[], integer, text[], text[], text[], text[], timestamp with time zone, date, date, text[], text[]);
