-- Add metadata filters: attacker, target, affected_party, topics, and user-selected country filtering.

-- Drop old 12-param overload so the new signature replaces it cleanly
DROP FUNCTION IF EXISTS public.get_feed_events(text[], integer, text[], text[], text[], text[], timestamp with time zone, date, date, text[], text[], text);
...
AS $function$
  ...
  WHERE (p_query IS NOT NULL OR p_countries IS NULL OR e.data->>'location_country' = ANY(p_countries))
