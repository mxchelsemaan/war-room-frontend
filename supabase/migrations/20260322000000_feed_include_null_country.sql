-- Include events with NULL location_country in the feed so that
-- political statements and other X-sourced events without geocoding
-- are not silently dropped by the country filter.

-- Drop the current 17-param overload so the new signature replaces it
DROP FUNCTION IF EXISTS public.get_feed_events(text[], integer, text[], text[], text[], text[], timestamp with time zone, date, date, text[], text[], text, text[], text[], text[], text[], text[]);

CREATE OR REPLACE FUNCTION public.get_feed_events(p_countries text[] DEFAULT NULL::text[], p_limit integer DEFAULT 30, p_types text[] DEFAULT NULL::text[], p_severities text[] DEFAULT NULL::text[], p_regions text[] DEFAULT NULL::text[], p_source_types text[] DEFAULT NULL::text[], p_cursor timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_weapon_systems text[] DEFAULT NULL::text[], p_handles text[] DEFAULT NULL::text[], p_query text DEFAULT NULL::text, p_filter_countries text[] DEFAULT NULL::text[], p_attackers text[] DEFAULT NULL::text[], p_targets text[] DEFAULT NULL::text[], p_affected_parties text[] DEFAULT NULL::text[], p_topics text[] DEFAULT NULL::text[])
...
AS $function$
  ...
  WHERE (p_query IS NOT NULL OR p_countries IS NULL OR e.data->>'location_country' = ANY(p_countries) OR e.data->>'location_country' IS NULL)
    ...
    AND (p_filter_countries IS NULL OR e.data->>'location_country' = ANY(p_filter_countries))
    AND (p_attackers IS NULL OR e.data->>'attacker' = ANY(p_attackers))
    AND (p_targets IS NULL OR e.data->>'target' = ANY(p_targets))
    AND (p_affected_parties IS NULL OR e.data->>'affected_party' = ANY(p_affected_parties))
    AND (p_topics IS NULL OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(COALESCE(e.data->'topics','[]'::jsonb)) t WHERE t = ANY(p_topics)))
    AND (p_query IS NULL OR (
      ...
      OR EXISTS (
        SELECT 1 FROM enriched.named_entities ne
        WHERE ne.source_type = e.source_type AND ne.source_id = e.source_id
        AND (
          ne.entity_value ILIKE '%' || p_query || '%'
          OR ne.entity_normalized ILIKE '%' || p_query || '%'
          OR similarity(ne.entity_normalized, lower(p_query)) > 0.3
        )
      )
    ))
    AND NOT (
