-- Remove lat/lng filter from get_feed_events so coordinate-less events
-- (political statements, diplomatic meetings, legislation, etc.) appear in the feed.
-- Map RPCs (get_map_markers, get_new_markers_since) keep the coordinate requirement.

CREATE OR REPLACE FUNCTION public.get_feed_events(p_countries text[] DEFAULT NULL::text[], p_limit integer DEFAULT 30, p_types text[] DEFAULT NULL::text[], p_severities text[] DEFAULT NULL::text[], p_regions text[] DEFAULT NULL::text[], p_source_types text[] DEFAULT NULL::text[], p_cursor timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_weapon_systems text[] DEFAULT NULL::text[], p_handles text[] DEFAULT NULL::text[])
